################################################################################
##' @title Null vs model comparison of grid cell selection; zed2
##' @author Robin Elahi
...
## In this null model, I will select oases in each jurisdiction separately
# Determine # of oases in each jurisdiction
df_oases <- dat %>% count(OCEAN, JURISDICTION, oasis_present) %>% 
  spread(key = oasis_present, value = n) %>% 
...
dat2 <- df_oases %>%
  left_join(dat, ., by = c("OCEAN", "JURISDICTION"))
...
get_n_cells_jurisdiction <- function(dat){
  dat %>% 
    group_by(JURISDICTION) %>% 
    do(get_n_cells_null(.)) %>% 
    ungroup() %>%
...
n_cells_null <- replicate(n = n_draws, get_n_cells_jurisdiction(dat2))
