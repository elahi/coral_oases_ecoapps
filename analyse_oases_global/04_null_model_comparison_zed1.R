################################################################################
##' @title Null vs model comparison of grid cell selection; zed1
##' @author Robin Elahi
...
## In this null model, I will select oases in each ocean separately
# Determine # of oases in each jurisdiction
df_oases <- dat %>% count(OCEAN, oasis_present) %>% 
...
get_n_cells_ocean <- function(dat){
  dat %>% 
    group_by(OCEAN) %>% 
    do(get_n_cells_null(.)) %>% 
    ungroup() %>%
...
n_cells_null <- replicate(n = n_draws, get_n_cells_ocean(dat2))
