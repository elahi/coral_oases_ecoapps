...
##### Marianas #####
statDat <- dat %>% filter(REGION == "MHI") %>% droplevels()
statDat %>% 
...
statDat <- statDat %>% 
  mutate(REGION = ifelse(REGION_SUB == "Tinian" | REGION_SUB == "Aguijan", 
                         "Tinian_Aguijan", REGION))

statDat %>% 
  count(JURISDICTION, REGION)

# Plot
my_jurisdiction <- "Main Hawaiian Islands"
unique(statDat$REGION)
statDat$REGION <- factor(statDat$REGION, 
                         levels = c("Niihau", "Kauai", "Oahu", 
