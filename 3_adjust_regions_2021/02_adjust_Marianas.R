...
##### Marianas #####
statDat <- dat %>% filter(REGION == "MARIAN") %>% droplevels()
statDat %>% 
...
# Create new REGION for Marianas
statDat <- statDat %>% 
  mutate(REGION = ifelse(REGION_SUB == "Farallon de Pajaros" | REGION_SUB == "Maug" | 
                           REGION_SUB == "Asuncion", 
                         "Marianas_upper", REGION))
statDat <- statDat %>% 
  mutate(REGION = ifelse(REGION_SUB == "Pagan" | REGION_SUB == "Alamagan" | 
                           REGION_SUB == "Guguan" | REGION_SUB == "Sarigan", 
                         "Marianas_middle", REGION))

statDat <- statDat %>% 
  mutate(REGION = ifelse(REGION_SUB == "Saipan" |
                         REGION_SUB == "Tinian" | REGION_SUB == "Aguijan", 
                         "Marianas_lower", REGION))

statDat %>% 
  count(JURISDICTION, REGION)

# Plot
my_jurisdiction <- "Marianas"
unique(statDat$REGION)
statDat$REGION <- factor(statDat$REGION, 
                         levels = c("Marianas_upper", "Marianas_middle", 
                                    "Marianas_lower", "Rota", "Guam"))
...
ggsave(paste(path_to_figs, "map2021_", my_jurisdiction, ".pdf", sep = ""), height = 7, width = 5)
